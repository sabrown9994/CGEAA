  ...
  createCmd
    ...
    .action((name: string, opts: { file?: string; invoice?: string }) =>
      runCommand(program, async () => {
        ...
        // `pull` writes `_zdf` into debit-memo files, so a create off a pulled file must never
    ...
    .action((id: string, opts: { invoice?: string }) =>
      runCommand(program, async () => {
        ...
        if (target.found) {
          ...
          // Debit memo is natural-keyed (memoNumber, tenant-assigned — NOT preserved from the
