...
| create | `POST /v1/invoices` | not supported (tenant config — see TODO.md) |
...
- **Not supported (tenant config — see TODO.md)**: `create invoice` requires Finance > Manage Non-Subscription Items settings (revenue recognition accounting codes) to be configured, which they are not on this tenant. The CLI fails fast with a clear error before making any network call.
