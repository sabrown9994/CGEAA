...
| create | `POST /v1/invoices` | Standalone invoice create; **tenant-gated** (see below) |
...
- Most invoices are still generated by bill runs, not `create invoice` directly.
- **BLOCKED-BY-TENANT-CONFIG on intQA**: `create invoice` requires Finance > Manage Non-Subscription Items settings (revenue recognition accounting codes) to be configured, which they are not on intQA. The command is implemented correctly but could not be live-verified end-to-end on this tenant.
...
| create | `POST /v1/credit-memos` | **Tenant-gated** (see below) |
...
- `create credit-memo` depends on the Invoice Settlement feature being enabled and a source invoice to reference; it was **not live-tested on intQA** (tenant gating, same class of constraint as `create invoice`).
...
| create | `POST /v1/debit-memos` | **Tenant-gated** (see below) |
...
- `create debit-memo` has the same tenant gating as `create credit-memo` (Invoice Settlement feature); it was **not live-tested on intQA**.
